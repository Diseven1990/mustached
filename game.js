...
function drawBg(){
  ...
  if(score >= dangerScore){
    ctx.fillStyle = 'rgba(120,0,55,.10)';
    ctx.fillRect(0,0,W,H);
  }
}
...
function drawWindow(x,y,w,h,text){
  ...
  ctx.fillStyle = '#fff';
  ctx.font = '900 11px Courier New';
  ctx.fillText(text.slice(0,9), x+7, y+17);
...
function drawPlayer(){
  const img = player.frame ? img2 : img1;
  if(img.complete) {
    ctx.drawImage(
      img,
      Math.round(player.x),
...
function update(dt){
  ...
  player.y += (player.targetY - player.y) * .28;
  ...
  spawnCountdown -= dt;
  if(spawnCountdown <= 0){
    spawnCountdown = Math.max(1150 - score * 38, 680);
    spawnObstacle();
  }

  const speed = 2.55 + Math.min(score * .065, .70) + (score >= dangerScore ? .42 : 0);
